$(document).ready(function() {
    ...
    async function loadArtworkDetails() {
        ...
        try {
            // Cargar detalles de la obra
            const artworkResponse = await fetch(`backend/api/artworks.php?id=${currentArtworkId}`);
            const artworkData = await artworkResponse.json();

            if (artworkResponse.ok) {
                renderArtworkDetails(artworkData);
                loadComments(currentArtworkId); // Cargar comentarios después de la obra
                checkLikeAndFavoriteStatus(currentArtworkId); // Verificar si el usuario ya dio like/guardó
            } else {
    ...
    $commentForm.on('submit', async function(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                $commentText.val(''); // Limpiar el textarea
                loadComments(currentArtworkId); // Recargar comentarios
            } else {
    ...
    $likeButton.on('click', async function() {
        ...
        try {
            const response = await fetch('backend/api/artworks.php', {
                method: 'POST', // Usamos POST para añadir/quitar like
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userToken
                },
                body: JSON.stringify({
                    action: 'toggle_like', // Acción específica para el backend
                    artwork_id: currentArtworkId,
                    user_id: userId
                })
            });
            ...
            if (response.ok) {
                showMessage($commentMessage, data.message, 'success');
                // Actualizar contador de likes (necesitarás un endpoint para esto)
                // Por ahora, solo actualiza el texto del botón si la acción fue añadir/quitar
                ...
                if (data.like_count !== undefined) {
                    ...
                    // Si el backend no devuelve el contador, recargar la obra o hacer una petición adicional
                    loadArtworkDetails(); // Recargar para obtener el contador actualizado
                }
    ...
    $saveButton.on('click', async function() {
        ...
        try {
            const response = await fetch('backend/api/users.php', { // Usamos el endpoint de users para favoritos
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userToken
                ...
                body: JSON.stringify({
                    action: 'add_favorite', // Acción específica para el backend
                    userId: userId,
    ...
    async function checkLikeAndFavoriteStatus(artworkId) {
        if (!isUserLoggedIn()) {
            ...
            $likeButton.prop('disabled', false); // No deshabilitar, solo cambiar texto
            $saveButton.prop('disabled', false);
            return;
        }
        ...
        try {
            ...
            const likeStatusResponse = await fetch(`backend/api/artworks.php?action=get_like_status&artwork_id=${artworkId}&user_id=${userId}`, {
            ...
            const favoriteStatusResponse = await fetch(`backend/api/users.php?action=check_favorite&artwork_id=${artworkId}&user_id=${userId}`, {
                headers: { 'Authorization': 'Bearer ' + userToken }
            });
            ...
            // Si hay un error, no cambiar el texto del botón, o mostrar un mensaje
        } finally {
            $likeButton.prop('disabled', false);
            $saveButton.prop('disabled', false);
        }
